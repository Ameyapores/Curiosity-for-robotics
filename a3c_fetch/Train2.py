...
def normal(x, mu, sigma):
    pi = np.array([math.pi])
    pi = torch.from_numpy(pi).type(torch.cuda.FloatTensor)
...
def train(rank, args, shared_model, counter, lock, optimizer=None, select_sample=True):
    ...
    for num_iter in count():
        ...
        for step in range(args.num_steps):
            ...
            eps = torch.randn(mu.size()).type(FloatTensor)
            pi = np.array([math.pi])
            pi = torch.from_numpy(pi).type(FloatTensor)
            action = (mu + sigma.sqrt() * eps).data
            act = Variable(action)
            prob = normal(act, mu, sigma)
            action = torch.clamp(action, -1.0, 1.0)
            entropy = 0.5 * ((sigma * 2 * pi.expand_as(sigma)).log() + 1).sum(-1, keepdim=True)
            #print (entropy)
            log_prob = (prob + 1e-6).log().sum(-1, keepdim=True)
            #print(log_prob)
